...
local function telescope_package_search(callback, opts)
  ...
  local package_root = HtProjectUtil.match_package_root(file)
...
HsProjectTools.root_dir = function(project_file)
  return HtProjectUtil.match_cabal_project_root(project_file)
    or HtProjectUtil.match_stack_project_root(project_file)
    or HtProjectUtil.match_package_root(project_file)
    or HtProjectUtil.match_hie_yaml(project_file)
end
...
HsProjectTools.open_package_yaml = function()
  vim.schedule(function()
    local file = vim.api.nvim_buf_get_name(0)
    local result = HtProjectUtil.get_package_yaml(file)
    if not result then
      local context = ''
      if HtProjectUtil.is_cabal_project(file) then
...
HsProjectTools.open_package_cabal = function()
  vim.schedule(function()
    local file = vim.api.nvim_buf_get_name(0)
    if vim.fn.filewritable(file) ~= 0 and not HtProjectUtil.is_cabal_project(file) then
      vim.notify('HsPackageCabal: Not a cabal project?', vim.log.levels.ERROR)
      return
    end
    local result = HtProjectUtil.get_package_cabal(file)
    if not result then
...
HsProjectTools.open_project_file = function()
  vim.schedule(function()
    local file = vim.api.nvim_buf_get_name(0)
    local stack_project_root = HtProjectUtil.match_stack_project_root(file)
    if stack_project_root then
      vim.cmd('e ' .. stack_project_root .. '/stack.yaml')
      return
    end
    local cabal_project_root = HtProjectUtil.match_cabal_multi_project_root(file)
    if cabal_project_root then
      vim.cmd('e ' .. cabal_project_root .. '/cabal.project')
      return
    end
    local package_cabal = HtProjectUtil.get_package_cabal(file)
